  ...
  Mutation: {
    ...
    updateTodo: (_, { input }) => {
      todos
        ...
        .map((todo) => (todo.title = input.title));
      todos
        .filter((todo) => todo.id == input.id)
        .map((todo) => (todo.completed = input.completed));
      return `Todo with ${input.id} successfully updated`;
    },
    deleteTodo: (_, { input }) => {
      todos.filter((todo) => todo.id != input.id);
      return `Todo with ${input.id} successfully deleted`;
    },
  },
};
